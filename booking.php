...
<?php 
    ...
    if(isset($_POST['book'])){
        ...
        if(empty($bplace)|| empty($bdate)|| empty($dur)|| empty($phno)|| empty($des)|| empty($rdate)){
            ...
            if($bdate<$rdate){
            $price=($dur*$bikeprice);
            $sql="insert into booking (BIKE_ID,EMAIL,BOOK_PLACE,BOOK_DATE,DURATION,PHONE_NUMBER,DESTINATION,PRICE,RETURN_DATE) values($bikeid,'$uemail','$bplace','$bdate',$dur,$phno,'$des',$price,'$rdate')";
